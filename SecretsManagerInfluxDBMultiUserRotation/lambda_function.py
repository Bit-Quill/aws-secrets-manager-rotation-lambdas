...
def lambda_handler(event, context):
    ...
    if SECRETS_MANAGER_ENDPOINT not in os.environ:
        raise KeyError("SECRETS_MANAGER_ENDPOINT environment variable not set in the environment variables.")
    ...
    secrets_client = boto_session.client("secretsmanager", endpoint_url=secret_manager_endpoint)

    create_auth_enabled = False

    # By default user and token creation is not enabled
    if AUTH_CREATION_ENABLED in os.environ and os.environ[AUTH_CREATION_ENABLED] == "true":
    ...
    if version_id not in versions:
        logger.error("Secret version %s has no stage for rotation of secret %s." % (version_id, arn))
        raise ValueError("Secret version %s has no stage for rotation of secret %s." % (version_id, arn))
    if CURRENT_STAGE in versions[version_id]:
        logger.info("Secret version %s already set as AWSCURRENT for secret %s." % (version_id, arn))
        return
    elif PENDING_STAGE not in versions[version_id]:
        logger.error("Secret version %s not set as AWSPENDING for rotation of secret %s." % (version_id, arn))
        raise ValueError("Secret version %s not set as AWSPENDING for rotation of secret %s." % (version_id, arn))

    if step == CREATE_STEP:
        create_secret(secrets_client, boto_session, arn, version_id, create_auth_enabled)
...
def get_connection(endpoint_url, secret_dict, arn, step):
    ...
    try:
        conn = influxdb_client.InfluxDBClient(
            ...
            debug=False,
            verify_ssl=True
        ) if INFLUXDB_TOKEN in secret_dict else influxdb_client.InfluxDBClient(
            url="https://" + endpoint_url + ":8086",
...
def create_secret(secrets_client, boto_session, arn, version_id, create_auth_enabled):
    ...
    try:
        ...
        operator_secret_dict = get_operator_dict(secrets_client, current_secret_dict, CURRENT_STAGE, arn)
        with get_connection(get_db_info(current_secret_dict[INFLUXDB_INSTANCE_IDENTIFIER], boto_session),
                            operator_secret_dict, arn, CREATE_STEP) as conn:
            ...
            if INFLUXDB_TOKEN_TYPE in current_secret_dict:
                ...
                org = next((org for org in conn.organizations_api().find_organizations() if
                            org.name == current_secret_dict[INFLUXDB_ORG]), None)
                if org is None:
                    raise ValueError("Org does not exist to associate token value with")

                if current_secret_dict[INFLUXDB_TOKEN_TYPE] == "operator":
                    token_perms = create_operator_token_perms(all_permission_types)
                elif current_secret_dict[INFLUXDB_TOKEN_TYPE] == "allAccess":
                    token_perms = create_all_access_token_perms(all_permission_types, org.id, conn.users_api().me().id)
                else:  # custom
                ...
                final_token_perm = validate_current_token(conn, current_secret_dict, token_perms)
                create_token(conn, current_secret_dict, final_token_perm, org)
            else:
                update_user_creds(secrets_client, boto_session, arn, conn, current_secret_dict, create_auth_enabled)

            secrets_client.put_secret_value(SecretId=arn, ClientRequestToken=version_id,
                                            SecretString=json.dumps(current_secret_dict), VersionStages=[PENDING_STAGE])

    logger.info("create_secret: Successfully created new authorization for ARN %s and version %s." % (arn, version_id))


def update_user_creds(secrets_client, boto_session, arn, conn, current_secret_dict, create_auth_enabled):
    ...
    user = next((user for user in users.users if user.name == current_secret_dict[INFLUXDB_USERNAME]), None)
    ...
    if user is not None and INFLUXDB_PASSWORD not in current_secret_dict:
        raise ValueError(
            "User %s password has not been set yet, set the password before performing a rotation." % current_secret_dict[
                INFLUXDB_USERNAME])

    if user is not None and INFLUXDB_PASSWORD in current_secret_dict:
        with get_connection(get_db_info(current_secret_dict[INFLUXDB_INSTANCE_IDENTIFIER], boto_session),
                            current_secret_dict, arn, CREATE_STEP) as test_user_client:
            # Ensure user defined in secret can authenticate
            test_user_client.users_api().me()

    if user is None:
        if not create_auth_enabled:
            raise ValueError("Authentication creation has not been enabled to allow the lambda function to create users")

        logger.info("User has not been created yet, creating new user %s" % current_secret_dict[INFLUXDB_USERNAME])
        ...
        if INFLUXDB_ORG in current_secret_dict:
            ...
            conn.organizations_api()._organizations_service.post_orgs_id_members(org.id, user)
...
def append_organization_scoped_permission(token_perms, perm_type, action, org_id):
    ...
    """
    token_perms.append(
        influxdb_client.Permission(resource=influxdb_client.PermissionResource(type=perm_type, org_id=org_id),
                                   action=action))
...
def append_organization_permission(token_perms, perm_type, action, org_id):
    ...
    """
    token_perms.append(
        influxdb_client.Permission(resource=influxdb_client.PermissionResource(id=org_id, type=perm_type),
                                   action=action))
...
def append_user_permission(token_perms, perm_type, action, user_id):
    ...
    """
    token_perms.append(
        influxdb_client.Permission(resource=influxdb_client.PermissionResource(id=user_id, type=perm_type),
                                   action=action))


def append_organization_and_bucket_scoped_permission(token_perms, perm_type, action, org_id, bucket_id):
    """
    ...
    """
    token_perms.append(influxdb_client.Permission(
        resource=influxdb_client.PermissionResource(name=bucket_id, type=perm_type, org_id=org_id), action=action))
...
def append_non_scoped_permission(token_perms, perm_type, action):
    ...
    """
    token_perms.append(
        influxdb_client.Permission(resource=influxdb_client.PermissionResource(type=perm_type), action=action))
...
def validate_current_token(conn, current_secret_dict, token_perms):
    ...
    if (INFLUXDB_TOKEN_TYPE in current_secret_dict and current_secret_dict[INFLUXDB_TOKEN_TYPE] == "operator"
            and INFLUXDB_TOKEN not in current_secret_dict):
        raise ValueError("Operator tokens cannot be created without an already existing operator token")

    # If no value is set for the token, and the token is not of type operator we will create the new token
    if INFLUXDB_TOKEN_TYPE in current_secret_dict and INFLUXDB_TOKEN not in current_secret_dict:
        return token_perms

    # Ensure the already created token is the permission set that we use when creating the new token
    if INFLUXDB_TOKEN_TYPE in current_secret_dict and INFLUXDB_TOKEN in current_secret_dict:
        authorizations = conn.authorizations_api().find_authorizations()
        current_auth = next((auth for auth in authorizations if auth.token == current_secret_dict[INFLUXDB_TOKEN]),
                            None)
        if current_auth is None:
            raise ValueError("InfluxDB token is incorrect (can't find matching token) or empty operator token is "
                             "set")
        else:
        ...
        raise ValueError("InfluxDB token or token type is sent incorrectly, ensure type and value are defined in "
                             "Secret Manager")
...
def create_operator_token_perms(all_permission_types):
    """

    Create the set of permissions defined for an operator token.

    Args:
        all_permission_types (list): List of all available permission types
...
def create_all_access_token_perms(all_permission_types, org_id, user_id):
    """
...
def create_custom_token_perms(current_secret_dict, org_id):
    ...
    if INFLUXDB_PERMISSIONS in current_secret_dict:
        for perm in current_secret_dict[INFLUXDB_PERMISSIONS]:
            append_organization_scoped_permission(token_perms, get_type_from_perm_string(perm),
                                                  get_action_from_perm_string(perm), org_id)
...
def test_secret(secrets_client, boto_session, arn, version_id):
    ...
    current_secret_dict = get_secret_dict(secrets_client, arn, CURRENT_STAGE)
    pending_secret_dict = get_secret_dict(secrets_client, arn, PENDING_STAGE, version_id)
    operator_secret_dict = get_operator_dict(secrets_client, pending_secret_dict, CURRENT_STAGE, arn)

    # Verify pending authentication can successfully authenticate
    with get_connection(get_db_info(pending_secret_dict[INFLUXDB_INSTANCE_IDENTIFIER], boto_session),
                        pending_secret_dict, arn, TEST_STEP) as pending_user_client:
        pending_user_client.organizations_api().find_organizations()

    with get_connection(get_db_info(current_secret_dict[INFLUXDB_INSTANCE_IDENTIFIER], boto_session),
                        operator_secret_dict, arn, CREATE_STEP) as operator_client:
        if INFLUXDB_TOKEN in pending_secret_dict and INFLUXDB_TOKEN in current_secret_dict:
            ...
            current_auth = next((auth for auth in authorizations if auth.token == current_secret_dict[INFLUXDB_TOKEN]),
                                None)
            pending_auth = next((auth for auth in authorizations if auth.token == pending_secret_dict[INFLUXDB_TOKEN]))

            # Validate current and pending authorizations have the same permissions if there was a previously set
            # auth token
            if current_auth is not None and (not current_auth.permissions == pending_auth.permissions):
                raise ValueError("Current and pending tokens failed permissions test for secret ARN %s" % arn)
...
def finish_secret(secrets_client, boto_session, arn, version_id):
    ...
    secrets_client.update_secret_version_stage(SecretId=arn, VersionStage=CURRENT_STAGE, MoveToVersionId=version_id,
    ...
    logger.info("finish_secret: Successfully set AWSCURRENT stage to version %s for secret %s." % (version_id, arn))
...
def get_secret_dict(secrets_client, arn, stage, version_id=None):
    ...
    if version_id:
        secret = secrets_client.get_secret_value(SecretId=arn, VersionId=version_id, VersionStage=stage)
    else:
    ...
    if INFLUXDB_TOKEN_TYPE in secret_dict:
        required_fields = [INFLUXDB_ENGINE, INFLUXDB_TOKEN_TYPE, INFLUXDB_INSTANCE_IDENTIFIER,
                           INFLUXDB_OPERATOR_TOKEN_ARN]
    else:
        required_fields = [INFLUXDB_ENGINE, INFLUXDB_USERNAME, INFLUXDB_INSTANCE_IDENTIFIER,
                           INFLUXDB_OPERATOR_TOKEN_ARN]
    ...
    if INFLUXDB_TOKEN_TYPE in secret_dict and secret_dict[
        INFLUXDB_TOKEN_TYPE] == "operator" and INFLUXDB_TOKEN not in secret_dict:
...
def get_operator_dict(secrets_client, secret_dict, stage, arn):
    ...
    operator_secret = secrets_client.get_secret_value(SecretId=secret_dict[INFLUXDB_OPERATOR_TOKEN_ARN],
                                                      VersionStage=stage)
