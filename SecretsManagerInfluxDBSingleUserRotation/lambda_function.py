...
def create_secret(secrets_client, boto_session, arn, version_id):
    ...
    try:
        ...
        with get_connection(get_db_info(current_secret_dict[INFLUXDB_INSTANCE_IDENTIFIER], boto_session),
                            current_secret_dict, arn, CREATE_STEP) as conn:
            ...
            if INFLUXDB_TOKEN in current_secret_dict:
                ...
                update_user_creds(secrets_client, boto_session, arn, conn, current_secret_dict)
...
def update_user_creds(secrets_client, boto_session, arn, conn, current_secret_dict):
    ...
    if INFLUXDB_PASSWORD not in current_secret_dict:
        raise ValueError("User %s password has not been set yet, set the password before performing a rotation." %
                         current_secret_dict[INFLUXDB_USERNAME])

    if user is None:
        raise ValueError(
            "User %s has not been created, create the user before performing a rotation" % current_secret_dict[
                INFLUXDB_USERNAME])
...
def validate_current_token(conn, current_secret_dict):
    ...
    current_auth = next((auth for auth in authorizations if auth.token == current_secret_dict[INFLUXDB_TOKEN]), None)
...
def create_token(conn, current_secret_dict, token_perms):
    ...
    org = next(
        (org for org in conn.organizations_api().find_organizations() if org.name == current_secret_dict[INFLUXDB_ORG]),
        None)
    if org is None:
...
def test_secret(secrets_client, boto_session, arn, version_id):
    ...
    with get_connection(get_db_info(current_secret_dict[INFLUXDB_INSTANCE_IDENTIFIER], boto_session),
                        pending_secret_dict, arn, CREATE_STEP) as client:
        if INFLUXDB_TOKEN in pending_secret_dict and INFLUXDB_TOKEN in current_secret_dict:
            ...
            if current_auth is None or pending_auth is None:
                raise ValueError("Current or pending token does not exist for secret ARN %s" % arn)

            # Validate current and pending tokens have the same permissions
            if current_auth.permissions != pending_auth.permissions:
                raise ValueError("Current and pending tokens do not have the same permissions for secret ARN %s" % arn)
...
def finish_secret(secrets_client, boto_session, arn, version_id):
    ...
    if INFLUXDB_TOKEN in pending_secret_dict and INFLUXDB_TOKEN in current_secret_dict:
        with get_connection(get_db_info(current_secret_dict[INFLUXDB_INSTANCE_IDENTIFIER], boto_session),
                            pending_secret_dict, arn, CREATE_STEP) as operator_client:
...
def get_secret_dict(secrets_client, arn, stage, version_id=None):
    ...
    if INFLUXDB_TOKEN not in secret_dict and INFLUXDB_USERNAME not in secret_dict and INFLUXDB_PASSWORD not in secret_dict:
        raise KeyError("No credentials were provided to authenticate with the DB instance")

    # Run semantic validations for secrets
    if INFLUXDB_TOKEN in secret_dict:
        required_fields = [INFLUXDB_ENGINE, INFLUXDB_ORG, INFLUXDB_INSTANCE_IDENTIFIER, INFLUXDB_TOKEN]
    else:
        required_fields = [INFLUXDB_ENGINE, INFLUXDB_INSTANCE_IDENTIFIER, INFLUXDB_USERNAME, INFLUXDB_PASSWORD]
