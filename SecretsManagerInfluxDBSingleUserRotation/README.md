...
## Permissions
The `InfluxDB Single-user Rotation Lambda` rotates existing operator tokens with the same permissions that are defined for the token in the DB instance. This is to avoid any scenarios where a token of non-operator type can escalate to that of an operator during the rotation.
...
```
{
    ...
    "token": "string"                           // mandatory operator token value
}
```
